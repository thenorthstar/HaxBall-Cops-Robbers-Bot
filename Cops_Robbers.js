            ...
            if (Math.abs(reds.length - blues.length) == 1) {
                ...
                if (spectators.length == 0) {
                    ...
                    spectators.forEach(s => {
                        Math.sign(difference2) == 1 ? room.setPlayerTeam(s.id, 1) : room.setPlayerTeam(s.id, 2);
                    });
                }
            }
        }
...
function fillCircles() {
    var circles_red = roomObject.prohibitedPoints[roomObject.teams[1]].circle;
    var circles_blue = roomObject.prohibitedPoints[roomObject.teams[2]].circle;
    var circles = [circles_red, circles_blue];
    var players = room.getPlayerList().filter(p => room.getPlayerDiscProperties(p.id) != null);

    for (var i = 0; i < circles.length; i++) {
        for (var c in circles[i]) {
            var player = players.filter(p => p.team == i + 1).find(p => pointDistance({ x: c.x, y: c.y }, room.getPlayerDiscProperties(p.id)) < c.radius - room.getPlayerDiscProperties(p.id).radius);

            if (player != null) {
                var index = JMap.discs.findIndex(d => d.pos[0] == c.x && d.pos[1] == c.y);
                if (room.getDiscProperties(index + 1).cMask & bias_cMasks[player.team - 1] == 0) {
                    room.setDiscProperties(index + 1, { cMask: room.getDiscProperties(index + 1).cMask | bias_cMasks[player.team - 1] });
                }
                var others = players.filter(p => p.team == player.team && p.id != player.id && room.getPlayerDiscProperties(p.id).cGroup & bias_cGroups[p.team - 1] == 0);
                others.forEach(p => {
                    room.setPlayerDiscProperties(p.id, { cGroup: room.getPlayerDiscProperties(p.id).cGroup | bias_cGroups[p.team - 1] });
                });
            }
            else {
                var others = players.filter(p => p.team == i + 1 && room.getPlayerDiscProperties(p.id).cGroup & bias_cGroups[p.team - 1] != 0);
                others.forEach(p => {
...
function fillGateArea() {
    ...
    if (players.length >= roomObject.tolerances.bugging.blue.gate) {
        ...
        var _players = room.getPlayerList().filter(p => room.getPlayerDiscProperties(p.id) != null && isPlayerOnGate(p) == false && room.getPlayerDiscProperties(p.id).cGroup & cf.score != 0);
        _players.forEach(p => {
            room.setPlayerDiscProperties(p.id, { cGroup: cGroups[p.team - 1][Number(roomObject.collision)] });
        });
    }
